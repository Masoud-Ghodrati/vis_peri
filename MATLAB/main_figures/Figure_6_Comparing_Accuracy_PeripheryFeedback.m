...
FIGURE_DIMENSION = [0 0 970 325;
    0 0 970 325]; % dimesion of the printed figure
...
for iPlot = 1 : 1  % number of subplot
    ...
    for iCategory_Level = 1 : length(Results_Periphery_Matched)
        ...
        for iSOA = 1 : 5  % 4 SOA + without noise
            ...
            if iSOA == 1
                subject_Data_Matrix_temp = Results_Periphery_Matched{iCategory_Level}.PerformanceAll;
                subject_Data_Matrix_temp = subject_Data_Matrix_temp(:);
            else
                subject_Data_Matrix_temp = Results_Feedback_Matched{iCategory_Level}.PerformanceAll(:, :, iSOA-1);
                subject_Data_Matrix_temp = subject_Data_Matrix_temp(:);
            end
...
for iCategory_Level = 1 : length(Results_Periphery_Matched)
    ...
    for iEccentricity = 1 : 4
        ...
        for iSOA = 1 : 4   % 4 SOA + without noise
            ...
            subject_Data_Matrix = [];
            if iEccentricity == 1
                subject_Data_Matrix = Results_Feedback_Matched{iCategory_Level}.PerformanceAll(:, Eccentricity_Array(iEccentricity, 1), iSOA);
            else
                subject_Data_Matrix = Results_Feedback_Matched{iCategory_Level}.PerformanceAll(:, Eccentricity_Array(iEccentricity, :), iSOA);
            end
        ...
        subject_Data_Matrix = [];
        if iEccentricity == 1
            subject_Data_Matrix = Results_Periphery_Matched{iCategory_Level}.PerformanceAll(:, Eccentricity_Array(iEccentricity, 1));
        else
            subject_Data_Matrix = Results_Periphery_Matched{iCategory_Level}.PerformanceAll(:, Eccentricity_Array(iEccentricity, :));
        end
...
if SAVE_PDF == true
    if WANT_LEGEND == true
        print('-dpdf', PDF_RESOLUTION, [save_PDF_Path 'Figure_6_Accuracy_Feedback_and_Periphery_AllSOAs_Legend_' date '.pdf'])
        winopen([save_PDF_Path 'Figure_6_Accuracy_Feedback_and_Periphery_AllSOAs_Legend_' date '.pdf'])
    elseif WANT_LEGEND==false
        print('-dpdf', PDF_RESOLUTION, [save_PDF_Path 'Figure_6_Accuracy_Feedback_and_Periphery_AllSOAs_' date '.pdf'])
        winopen([save_PDF_Path 'Figure_6_Accuracy_Feedback_and_Periphery_AllSOAs_' date '.pdf'])
    end
end
