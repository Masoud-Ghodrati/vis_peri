    ...
    for iSubject = 1 : length(current_Category)
        ...
        for isOA = 1 : length(unique_sOA)
            ...
            for iPosition = 1 : length(unique_Position)% positions
                ...
                fprintf([',    FL: ' num2str(this_subject_FalseAlarm(iPosition)) ' Hitt:'  num2str(this_subject_Hit(iPosition))])
...
for iCategory_Level = 1 : length(Results)
    ...
    for iExperimnet_InCategory = 1 : size(Results{iCategory_Level}.PerformanceAll,2)
        ...
        subject_dPrime_Matrix = squeeze(Results{iCategory_Level}.PerformanceAll(:,iExperimnet_InCategory, :));
        if sEM_AS_ERRORBAR == true
            h = errorbar(100:100:400, mean(subject_dPrime_Matrix), std(subject_dPrime_Matrix)./sqrt(size(subject_dPrime_Matrix,1)));
        elseif sEM_AS_ERRORBAR == false
            h = errorbar(100:100:400, mean(subject_dPrime_Matrix), std(subject_dPrime_Matrix));
        end
        h.Marker = 'o';
        ...
        subject_Hit_Matrix = squeeze(Results{iCategory_Level}.PerformanceClass_1(:,iExperimnet_InCategory, :));
        if sEM_AS_ERRORBAR == true
            h = errorbar(100:100:400, mean(subject_Hit_Matrix), std(subject_Hit_Matrix)./sqrt(size(subject_Hit_Matrix,1)));
        elseif sEM_AS_ERRORBAR == false
            h = errorbar(100:100:400, mean(subject_Hit_Matrix), std(subject_Hit_Matrix));
        end
        h.Marker = 'o';
        h.LineWidth = LINE_WIDTH;
        h.MarkerSize = MARKER_SIZE;
        ...
        subject_FalseAlarm_Matrix = squeeze(Results{iCategory_Level}.PerformanceClass_2(:,iExperimnet_InCategory, :));
        if sEM_AS_ERRORBAR
            h = errorbar(100:100:400, mean(subject_FalseAlarm_Matrix), std(subject_FalseAlarm_Matrix)./sqrt(size(subject_FalseAlarm_Matrix,1)));
        else
            h = errorbar(100:100:400, mean(subject_FalseAlarm_Matrix), std(subject_FalseAlarm_Matrix));
        end
        h.Marker = 'o';
        h.LineStyle = '--';
        h.LineWidth = LINE_WIDTH;
    ...
    if SAVE_PDF == true
        if WANT_LEGEND == true
            print('-dpdf', PDF_RESOLUTION, [save_PDF_Path 'dPrime_SOABased_FullScreen_FeedBack_Legend_'  Results{iCategory_Level}.TaskName  '_' date '.pdf'])
            winopen([save_PDF_Path 'dPrime_SOABased_FullScreen_FeedBack_Legend_'  Results{iCategory_Level}.TaskName  '_' date '.pdf'])
        elseif WANT_LEGEND==false
            print('-dpdf', PDF_RESOLUTION, [save_PDF_Path 'dPrime_SOABased_FullScreen_FeedBack_'  Results{iCategory_Level}.TaskName  '_' date '.pdf'])
            winopen([save_PDF_Path 'dPrime_SOABased_FullScreen_FeedBack_'  Results{iCategory_Level}.TaskName  '_' date '.pdf'])
        end
    end
end
...
for iCategory_Level = 1 : length(Results)
    ...
    for iExperimnet_InCategory = 1 : ceil(size(Results{iCategory_Level}.PerformanceAll,2)/2)
        ...
        if iExperimnet_InCategory == 1
            subject_dPrime_Matrix = squeeze(Results{iCategory_Level}.PerformanceAll(:,eccentricity_Select(iExperimnet_InCategory,1), :));
        else
            subject_dPrime_Matrix = [squeeze(Results{iCategory_Level}.PerformanceAll(:,eccentricity_Select(iExperimnet_InCategory,1), :));...
        ...
        if iExperimnet_InCategory==1
            subject_Hit_Matrix = squeeze(Results{iCategory_Level}.PerformanceClass_1(:,eccentricity_Select(iExperimnet_InCategory,1), :));
        else
            subject_Hit_Matrix = [squeeze(Results{iCategory_Level}.PerformanceClass_1(:,eccentricity_Select(iExperimnet_InCategory,1), :));...
        ...
        if iExperimnet_InCategory == 1
            subject_FalseAlarm_Matrix = squeeze(Results{iCategory_Level}.PerformanceClass_2(:,eccentricity_Select(iExperimnet_InCategory,1), :));
        else
            subject_FalseAlarm_Matrix = [squeeze(Results{iCategory_Level}.PerformanceClass_2(:,eccentricity_Select(iExperimnet_InCategory,1), :));...
    ...
    if SAVE_PDF == true
        if WANT_LEGEND == true
            print('-dpdf', PDF_RESOLUTION, [save_PDF_Path 'dPrime_SOABased_Half_Screen_FeedBack_Legend_'  Results{iCategory_Level}.TaskName  '_' date '.pdf'])
            winopen([save_PDF_Path 'dPrime_SOABased_Half_Screen_FeedBack_Legend_'  Results{iCategory_Level}.TaskName  '_' date '.pdf'])
        elseif WANT_LEGEND==false
            print('-dpdf', PDF_RESOLUTION, [save_PDF_Path 'dPrime_SOABased_Half_Screen_FeedBack_'  Results{iCategory_Level}.TaskName  '_' date '.pdf'])
            winopen([save_PDF_Path 'dPrime_SOABased_Half_Screen_FeedBack_'  Results{iCategory_Level}.TaskName  '_' date '.pdf'])
        end
    end
end